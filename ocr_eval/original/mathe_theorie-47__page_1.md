...
⇒ \(f(x)\) ist an \(x = 1\) nicht stetig, da nicht definiert an dieser Stelle (Definitionsecke) 
...
Unstetigkeitsstellen: • dicken im Definitionsbereich 
...
Bsp: 
\[ \bullet f(x) = \frac{x^2 - 1}{x + 1} \quad \text{bei } x_0 = -1 \text{ Definitionslecke} \] 
